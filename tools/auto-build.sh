...
if [[ "$1" == "-h" ]]; then
	...
	# Add Zabbix repo key
	lxc exec zbx-armhf-18 -- bash -c "curl https://repo.zabbix.com/zabbix-official-repo.key | apt-key add -"
	lxc exec zbx-armhf-20 -- bash -c "curl https://repo.zabbix.com/zabbix-official-repo.key | apt-key add -"
	lxc exec zbx-arm64-18 -- bash -c "curl https://repo.zabbix.com/zabbix-official-repo.key | apt-key add -"
	lxc exec zbx-arm64-20 -- bash -c "curl https://repo.zabbix.com/zabbix-official-repo.key | apt-key add -"
